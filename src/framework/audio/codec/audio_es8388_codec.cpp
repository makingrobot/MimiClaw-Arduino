 ...
 */
#include "config.h"
#if CONFIG_USE_AUDIO==1 && CONFIG_AUDIO_CODEC_ES8388==1

#include "audio_es8388_codec.h"
#include "src/framework/sys/log.h"

#define TAG "AudioEs8388Codec"

AudioEs8388Codec::AudioEs8388Codec(void* i2c_master_handle, i2c_port_t i2c_port, int input_sample_rate, int output_sample_rate,
    ...
    Log::Info(TAG, "AudioEs8388Codec initialized");
}

AudioEs8388Codec::~AudioEs8388Codec() {
...
bool AudioEs8388Codec::Init(const audio_config_t &config) {
...
void AudioEs8388Codec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din){
...
void AudioEs8388Codec::SetOutputVolume(int volume) {
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, volume));
    AudioCodec::SetOutputVolume(volume);
}

void AudioEs8388Codec::EnableInput(bool enable) {
...
void AudioEs8388Codec::EnableOutput(bool enable) {
...
uint32_t AudioEs8388Codec::Read(int16_t* dest, uint32_t samples) {
    if (input_enabled_) {
...
uint32_t AudioEs8388Codec::Write(const int16_t* data, uint32_t samples) {
    if (output_enabled_) {
