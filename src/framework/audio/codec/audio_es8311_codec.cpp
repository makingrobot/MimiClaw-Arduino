...
#define TAG "AudioEs8311Codec"

AudioEs8311Codec::AudioEs8311Codec(void* i2c_master_handle, i2c_port_t i2c_port, int input_sample_rate, int output_sample_rate,
    ...
    Log::Info(TAG, "AudioEs8311Codec initialized");
}

AudioEs8311Codec::~AudioEs8311Codec() {
...
bool AudioEs8311Codec::Init(const audio_config_t &config) {
...
void AudioEs8311Codec::UpdateDeviceState() {
...
void AudioEs8311Codec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
    assert(input_sample_rate_ == output_sample_rate_);
...
void AudioEs8311Codec::SetOutputVolume(int volume) {
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(dev_, volume));
    AudioCodec::SetOutputVolume(volume);
}

void AudioEs8311Codec::EnableInput(bool enable) {
...
void AudioEs8311Codec::EnableOutput(bool enable) {
...
uint32_t AudioEs8311Codec::Read(int16_t* dest, uint32_t samples) {
    if (input_enabled_) {
...
uint32_t AudioEs8311Codec::Write(const int16_t* data, uint32_t samples) {
    if (output_enabled_) {
